			...
			if data.get("row_changed"):
				if data.get("row_changed")[0][0] == "items":
					for d in data.get("row_changed")[0][-1]:
						if d[0] == "base_rate":
							old_value = d[-2].replace("SEK", "").replace(",",".").replace(" " ,"")
							currenct_value =d[-1].replace("SEK", "").replace(",",".").replace(" " ,"")
							value.append({
								...
								"percentage" : round((flt(old_value) - flt(currenct_value)) * 100 / flt(old_value), 2),
