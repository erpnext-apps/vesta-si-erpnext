...
frappe.xcall(
	'vesta_si_erpnext.vesta_si_erpnext.report.batch_wise_balance_and_inspection_new.batch_wise_balance_and_inspection_new.get_params'
	).then((data) => {
		for (d in data) {
			filters.push({
...
let item_dict = {};
frappe.query_reports["Batch-Wise Balance and Inspection New"] = {
	"filters": filters,
    ...
    onload: function(report) {
		...
		frappe.xcall(
		'vesta_si_erpnext.vesta_si_erpnext.report.batch_wise_balance_and_inspection_new.batch_wise_balance_and_inspection_new.create_stock_entry', {
			item_list: item_dict,
			}).then(stock_entry => {
		...
		report.page.add_inner_button(__("Create Certificate"), function() {
			...
			frappe.xcall(
			'vesta_si_erpnext.vesta_si_erpnext.report.batch_wise_balance_and_inspection_new.batch_wise_balance_and_inspection_new.create_certificate', {
				item_list: item_dict,
